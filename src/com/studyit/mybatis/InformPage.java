...
public class InformPage
{
	...
	// 페이징 처리 기능의 메소드
	// currentPage: 현재 표시할 페이지(내가 31페이지에 있으면 31 32 33..)
	// totalPage: 전체 페이지 수
	// lisUrl: 링크를 설정할 URL
	public String getIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		if (listUrl.indexOf("?") != -1) // 링크를 설정할 때 URL에 『?』가 들어있으면
			listUrl = listUrl + "&";
		else							// 링크를 설정할 때 URL에 『?』가 들어있지 않으면
			listUrl = listUrl + "?";
		//-- 예를 들어, 검색값이 존재하면 이미 request 값에
		//   searchKey값과 seatchValue 값이 들어있는 상황이므로
		//   『&』를 붙여서 추가해 주어야 한다.
		
		// currentPageSetup
		currentPageSetup = (currentPage / numPerBlock) * numPerBlock;
		//-- 만약 현재 페이지가 5페이지이고 (currentPage=5)
		//   리스트 하단에 보여줄 페이지 갯수사 10이면 (numPerBlock=10)
		...
		while( (page <= totalPage) && (page <= currentPageSetup + numPerBlock) )
		{
			if(page==currentPage)
				...
				// pageNum을 넘겨주는 이유는 목록으로 돌아갈 때 원래 있던 곳으로 돌아가기 위해
			page++;
		}
